...
vector<AnomalyReport> r; // the anomal detection after detect the anomalTest.csv
	...
	virtual void execute(){
		...
		while(menu){
			...
			if(optionN == "3"){ //WRONG SIZE - (23014 INSTEAD 6 ????)
			dio->write("Welcome to the Anomaly Detection Server.\nPlease choose an option:\n");
				for(Command* c : commandos){
					dioM->write(c->getDescription() + "\n");
				}				
...
class Command_1 : public Command{ //get from the user txt and upload csv files
	DefaultIO* dio1;
public:
	MacroCommand* m;
	string description;
...
class Command_2 : public Command{ //present the threshold that we or the user configure
	DefaultIO* dio2;
public:
	MacroCommand* m;
	string description;
	...
	virtual void execute() override{
		...
		while(!opt){
			...
			stringstream s(g);
			s>>thr;
			if(thr > 0 && thr < 1){ //if the user typed currect threshold, save it
				m->threshold = thr;
				opt = true;
			}else{
...
class Command_3 : public Command{ //from the given timesetries, find anomal correaltion to cf, and detect testCSV into r
	DefaultIO* dio3;
public:	
	MacroCommand* m;
	string description;
...
class Command_4 : public Command{ //present the anomal detections by timesteps and descreption
	DefaultIO* dio4;
public:
	MacroCommand* m;
	string description;
...
public:
	...
	virtual void execute(){
		...
		while(op){
    		...
    		for (int i; ss >> i;) { // get the min and max by vector
       	 	vect.push_back(i);    
       	 	if (ss.peek() == ',')
         		   ss.ignore();
    		}
