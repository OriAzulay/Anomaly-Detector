...
vector<AnomalyReport>  HybridAnomalyDetector::detect(const TimeSeries &ts){
	...
	for(correlatedFeatures cl: cf){
		...
		for(int j=0; j<ts.getVector().at(0).second.size(); j++){
			...
			if(cl.corrlation < threshold && cl.corrlation > corrlationMin){ // if correlation between 0.5->0.9, find by circle
			...
			}else{ //probably correlation above 0.9
